...
> **상태 — 소화 완료.** 아래 표의 **재작도 21종**과 **한 줄만 고치는 것 14종**은
> 전부 반영됐다. 이 표는 이제 남은 일 목록이 아니라 **무엇이 왜 틀렸었는지의
> 기록**이다. 무엇을 어떻게 고쳤는지는 `measured.md` 13~15절.
>
> 감사 기준 넷 중 셋은 이제 **계산이 잡는다.**
>
> | 기준 | 검사 |
> |---|---|
> | (가) 축척 | `tools/artlint.py` — 한 판 안의 px/m 이 1.6배 넘게 어긋나면 |
> | (나) 치수 부착 | `tools/anchor.py` — 치수선 양끝이 그려진 좌표에 3 px 안으로 닿는지 |
> | (다) 형상 | **사람이 봐야 한다.** 그려 놓고 격자에 늘어놓고 본다 (`sheet.py`) |
> | (라) 판 밖·겹침 | `tools/artlint.py` — 글자 상자·캡션 자리·이름표끼리 |
>
> 여기에 하나 더 붙었다. **사람 키** — 판의 px/m 로 역산해 1.45~1.95 m 를
> 벗어나면 잡는다(`tools/anchor.py`). 배율은 `1.7 × px/m ÷ 31`,
> 글리프 전체 높이는 26 이 아니라 **31** 이다.
...
/* 판은 이제 viewBox="-40 0 400 132" 다. 그림은 그대로 0~320 에 그리고
   좌우 40 씩은 배경만 채운다(카드 그림 상자가 320:132 보다 가로로 길어
   meet 로 넣으면 좌우가 비기 때문). 사고 경고 삼각형은 그 오른쪽 여백
   x 319~349 / y 6~42 로 옮겼으므로, 0~320 안은 전부 그림 자리다. */
