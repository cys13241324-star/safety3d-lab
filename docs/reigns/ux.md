...
> **2026-09-07 현재 — 열넷이 모두 닫혔다.** 파일을 뒤져 확인한 값이다.
>
> | 닫힘 | 어디에서 |
> |---|---|
> | D1 저장 · D13 오답 누적 | `reigns_run_v1` · `reigns_miss_v1` |
> | D2 틀린 규정 재출제 | `loadWeak()` — 놓친 조문에 가중치를 준다 |
> | D3 최근 카드 반복 방지 | `recentK` |
> | D4 포커스 함정 · D8 시트 높이 | `#sheet` 의 `hidden` 과 `max-height` |
> | D5 낭독 | `aria-live` |
> | D6 세로 잘림 · D7 카드 스크롤 | `100dvh` · `touch-action:pan-y` |
> | D9 게이지 | 값에 `⚠` 를 붙이고, 트랙에 `role="progressbar"` 와 `aria-valuetext` |
> | D10 흐린 글자 | `--dim:#7E8BA2` — card 4.86 · panel2 4.63 으로 AA(4.5) 를 넘긴다 |
> | D12 온보딩 | `reigns_briefed_v1` 과 0일차 브리핑 카드 |
> | D11 사이트 공통 테마 | `safety_theme` 부트 스크립트 · `:root[data-theme="light"]` · 상단바 `.bTheme` |
> | D14 형제 딥링크 | 판정 시트의 「3D 실습장에서 이 수치를 만져 본다」 — 덱 125장 중 **21장** |
>
> **D11 은 카드를 어두운 채로 남기는 쪽으로 풀었다.** 현장 그림 SVG 가 색을 날로
> 334곳 박고 있고 그중 300곳이 `DOC()` 안이라, 밝은 쪽으로 칠하려면 그림을 통째로
> 다시 그려야 한다. 밝은 종이 위의 어두운 화면은 사진과 같아서 그렇게 두는 편이
> 읽기도 낫다. `#card` 와 `#over` 안에서만 글자·선 색을 어두운 판 값으로 되돌린다.
> 밝은 판 색은 bg·panel·panel2 세 바탕 위에서 전부 AA(4.5)를 넘긴다.
>
> D10 은 이 표가 쓰인 뒤에 이미 고쳐졌다(`#6E7C95` → `#7E8BA2`). 2026-09-07 에
> 그것을 모르고 `index.html` 과 같은 값(`#8290AC`)으로 또 바꿨다가 되돌렸다 —
> **두 페이지는 바탕색이 달라 같은 값을 쓸 까닭이 없다.** 명도비는 짝이 되는
> 바탕 위에서 재야 한다.
